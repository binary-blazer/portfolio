...
export const metadata: Metadata = {
  ...
  icons: {
    icon: config.siteMetadata.favicon
  }
};
...
export default function Home() {
  ...
  useEffect(() => {
    setLanyardAvatar(
      window.localStorage.getItem('lanyard')
        ? JSON.parse(window.localStorage.getItem('lanyard')).discord_user.avatar
        : null
    );
    setLanyardUsername(
      window.localStorage.getItem('lanyard')
        ? JSON.parse(window.localStorage.getItem('lanyard')).discord_user.username
        : 'Loading'
    );
    setLanyardDiscriminator(
      window.localStorage.getItem('lanyard')
        ? JSON.parse(window.localStorage.getItem('lanyard')).discord_user.discriminator
        : '0000'
    );
  }, []);
        ...
        <div className="flex flex-col items-center" data-aos="fade-up">
              ...
              {config.IndexPage.lanyard.enabled && (
                <div className="flex items-center space-x-2 mb-2">
                  <div className="w-20 h-20 relative">
                    {lanyardAvatar == null ? (
                      <LegacyImage
                        className="rounded-md bg-white/10 button-text mr-20"
                        src={`https://cdn.discordapp.com/attachments/971049189377179718/1044214018618953769/unknown.png`}
                        height={700}
                        width={700}
                        draggable="false"
                      />
                      ...
                      <LegacyImage
                        className="rounded-md bg-white/10 button-text mr-20"
                        src={`https://cdn.discordapp.com/avatars/${config.IndexPage.lanyard.id}/${lanyardAvatar}.png`}
                        height={700}
                        width={700}
                        draggable="false"
                      />
                  ...
                  <div className="flex flex-col items-start">
                    <h1 className="text-start text-zinc-800 dark:text-zinc-200 transition-all duration-300 text-3xl leading-none font-semibold weight-900 button-text">
                      {lanyardUsername}
                    </h1>
                    {!lanyardDiscriminator === '0' && (
                      <span className="text-start text-2xl text-zinc-400 font-bold button-text mt-1">
                        #{lanyardDiscriminator}
                      </span>
                    )}
                  </div>
                </div>
              )}
              <h1 className="mx-auto font-semibold text-4xl md:text-5xl font-semibold sm:text-7xl text-black dark:text-white text-center button-text mt-5">
                <span className="text-primary">Hi there</span>, Im{' '}
                <span className="text-primary">{config.siteMetadata.author}</span>,
              </h1>
              <h1 className="leading-none text-2xl md:text-4xl font-bold mt-5 select-none">
                and im
                <TextLoop interval={3000} className="ml-2 max-w-2xl">
                  {texts.map((text, index) => (
                    <Tippy content={text.text} placement="top" key={index} delay={100} arrow={false}>
                      <div>
                        <h1 className="leading-none font-bold" style={{ color: text.color }}>
                          {text.text}
                        </h1>
                      </div>
                    </Tippy>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
                  <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.95 }}>
                    <button
                      ...
                      className="bg-primary/10 shadow-2xl hover:opacity-80 text-white font-display font-semibold text-lg px-10 py-3 rounded-md button button-text border border-primary/20 hover:bg-primary/20 active:bg-primary/30 transition duration-200"
                      href="/contact"
                    >
          ...
          {config.github.enabled && (
            <div className="flex flex-col items-center justify-center w-full px-10 text-center mt-40">
              <h1 className="text-5xl font-bold text-underline-2px button-text">Repositories</h1>
                    ...
                    <>
                      {reposData
                        ...
                        ?.map((_, __) => (
                          ...
                          >
                            <div onClick={() => window.open(_.html_url, '_blank')}>
          ...
          {config.projects.enabled && (
            <div className="flex flex-col items-center justify-center w-full px-10 text-center mt-40">
              <h1 className="text-5xl font-bold text-underline-2px button-text">Projects</h1>
              <p className="text-xl text-gray-600/90 dark:text-white/50 font-bold text-center mb-5 mt-3 button-text">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16">
                {_projectsData ? (
                    ...
                    projects?.map((p, i) => (
                      ...
                      >
                        ...
                        <div className="flex justify-center items-center grid grid-cols-1 mt-4 w-full">
                          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.92 }}>
                            {p.github === true ? (
                              <button
                                onClick={() => router.push(p.githubLink)}
                                target="_blank"
                                className="mt-2 bg-primary/10 shadow-2xl text-white font-display font-semibold text-lg w-full py-2 rounded-md mr-5 button button-text border border-primary/20"
                                href={p.githubLink}
                              >
                  ...
                  <div className="flex flex-col justify-center items-center">
                    ...
                    <span className="text-xl font-semibold button-text">Loading Projects...</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="flex flex-row justify-center items-center mt-10">
            <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.95 }}>
              <button
                ...
                className="bg-primary/10 shadow-2xl text-white font-display font-semibold text-lg px-10 py-3 rounded-md button button-text border border-primary/20 hover:bg-primary/20 active:bg-primary/30"
                href="/projects"
              >
                <i className="fas fa-project-diagram mr-2"></i> View All Projects
              </button>
            </motion.div>
          </div>
          ...
          {config.skills.enabled && (
            <div
              className="flex flex-col items-center justify-center w-full flex-2 px-20 text-center mt-20"
              data-aos="fade-right"
            >
              <h1 className="text-5xl font-bold text-underline-2px button-text">Skills</h1>
              <p className="text-xl text-gray-600/90 dark:text-white/50 font-bold text-center mb-5 mt-3 button-text">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 w-full gap-4 items-center mt-4">
                {_skills
                  ? skills
                    ? skills.map((_, __) => (
                        <motion.div whileHover={{ y: -4, transition: { duration: 0.3 } }} whileTap={{ y: 4 }}>
                          <div
                            key={__}
