...
export default function Post() {
  ...
  useEffect(() => {
    if (slug !== undefined) {
      axios.get(`/api/blog/find?slug=${slug}`).then(res => {
        setPost(res.data);
      });
    }
  }, [slug]);
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full h-full mt-20 mb-96">
      <Head>
        <title>
          {'Post:' + ' ' + post.name || 'Loading Blog Post'} | {config.siteMetadata.title}
        </title>
      </Head>

      <div
        className="flex flex-col items-center justify-center w-full h-full text-center mt-20"
      ...
      >
        ...
        >
          <div className="flex flex-col items-center justify-center w-full h-full select-none">
            <h1 className="text-left text-5xl underline underline-offset-4 font-bold mt-6">
          ...
          <img
            src={post.banner}
            className="w-full h-96 object-cover -mt-32 rounded-2xl opacity-10 select-none border border-2 border-primary/20 "
            draggable="false"
          />
          ...
          <div className="flex flex-col items-center justify-center w-full h-full flex-10 px-10 text-center">
              ...
              <div className="flex flex-col items-center justify-center w-full h-full flex-10 px-10 text-center">
                ...
                <div className="flex flex-row mt-2">
                  <img
                    ...
                    className="w-10 h-10 object-cover rounded-full opacity-100 select-none border border-2 border-primary/20"
                    draggable="false"
                  />
                ...
                <div className="flex flex-row mt-2">
                  {post.tags ? (
                    post.tags.slice(0, 4).map((tag, i) => (
                      <span
                        key={i}
                        className="mr-1 text-right text-sm bg-primary/10 border border-2 border-primary text-black dark:text-white font-bold px-2 py-1 rounded-md button button-text"
                      >
                        {tag}
                      </span>
                    ))
            ...
            <div
              className="flex flex-row items-center justify-center h-full flex-10 text-center mt-20"
