...
export default function Header() {
  ...
  useEffect(() => {
    ...
    setLanyardID(
      window.localStorage.getItem("lanyard")
        ? JSON.parse(window.localStorage.getItem("lanyard")).discord_user.id
        : null,
    );
  }, []);
  ...
  return <>
    <div
      className={`fixed top-0 left-0 right-0 z-50 transition-all max-w-7xl duration-300 ease-in-out mx-auto px-4 sm:px-6 lg:px-8 ${
        scrolled
          ? "bg-transparent"
          : "bg-black/70 top-2 left-0 right-0 md:top-4 md:left-8 md:right-8 rounded-none md:rounded-xl outline outline-2 outline-primary/70"
      }`}
      style={{ backdropFilter: scrolled ? "blur(0px)" : "blur(10px)" }}
    >
      <div className="flex justify-between items-center py-6 md:justify-start md:space-x-10">
        <div
          className="flex items-center gap-4 hover:opacity-80 transition duration-200 cursor-pointer"
          onClick={() => router.push("/")}
        >
          <div className="flex items-center gap-4">
            <img
              ...
              alt="avatar"
            />
            <a className="text-2xl font-bold transition-all duration-200 button-text text-black/90 dark:text-white/90 -ml-2">
        ...
        <div className="hidden md:flex space-x-10 items-center justify-center">
          ...
          >
            <Link href={page.href} key={page.name} legacyBehavior>
              ...
              >
                {router.pathname === page.href ? (
                  <span className="border-b-2 border-black dark:border-white padding button">
                    {" "}
                    <i
                      className={`${page.icon.active} text-gray-900 dark:text-gray-100 button`}
                    />{" "}
                    <span className="button">{page.name}</span>
                  </span>
                ) : (
                  <span>
                    {" "}
                    <i
                      className={`${page.icon.default} text-black dark:text-white padding-2 button`}
                    />{" "}
                    <span className="button">{page.name}</span>
                  </span>
                )}
          ...
          <div className="pt-5 pb-6 px-5">
            ...
            <div className="mt-6">
              <nav className="grid gap-y-8">
                {pages.map((page: any) => <Link href={page.href} key={page.name} legacyBehavior>
                  <a
